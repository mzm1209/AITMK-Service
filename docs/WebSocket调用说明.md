...
服务端启用了“坐席无操作自动离线”机制，当前阈值为 **30分钟**。
为避免前端停留在非会话页面时被误判，建议 Web 端定时上报心跳：

- `POST /api/agent/activity/ping?agentRowId={agentRowId}`
- 建议频率：每 20~30 秒一次（可与前端全局轮询复用）

> 说明：服务端会综合 WebSocket 订阅在线状态 + 心跳时间判定是否自动离线。
...
6. **定时心跳**：调用 `/api/agent/activity/ping`，降低误判离线概率
