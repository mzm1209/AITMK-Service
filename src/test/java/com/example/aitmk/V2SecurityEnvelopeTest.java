...
class V2SecurityEnvelopeTest {
 ...
 @Autowired ResourceRepository resources; @Autowired ConversationRepository conversations; @Autowired ChatMessageRepository messages;
 ...
 @Test void dashboardHttpContractUsesPluralNamesSecondsAndRealValues()throws Exception{
  Instant firstCustomerMessageAt=LocalDate.now(ZoneId.systemDefault()).atTime(1,0).atZone(ZoneId.systemDefault()).toInstant();
  Instant firstAgentReplyAt=firstCustomerMessageAt.plusSeconds(5);
	  ...
	  message(resource, conversation, PersistenceEnums.SenderType.CUSTOMER, firstCustomerMessageAt);
	  message(resource, conversation, PersistenceEnums.SenderType.AGENT, firstAgentReplyAt);
 ...
 private void message(ResourceEntity resource, ConversationEntity conversation, PersistenceEnums.SenderType sender, Instant at){
  var message=new ChatMessageEntity();message.setResourceId(resource.getId());message.setConversationId(conversation.getId());message.setCustomerPhone(resource.getCustomerPhone());message.setSenderType(sender);message.setSenderId(sender==PersistenceEnums.SenderType.CUSTOMER?null:DASHBOARD_OWNER_ID);message.setMessageType(PersistenceEnums.MessageType.TEXT);message.setSentStatus(sender==PersistenceEnums.SenderType.CUSTOMER?PersistenceEnums.SentStatus.DELIVERED:PersistenceEnums.SentStatus.SENT);message.setContent("dashboard contract");message.setCreatedAt(at);messages.saveAndFlush(message);
 }
}
