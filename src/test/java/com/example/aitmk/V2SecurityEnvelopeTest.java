...
class V2SecurityEnvelopeTest {
 ...
 @Test void dashboardHttpContractUsesPluralNamesSecondsAndRealValues()throws Exception{
  var resource=new ResourceEntity();resource.setCustomerPhone("dashboard-contract-phone");resource.setResourceStatus(PersistenceEnums.ResourceStatus.ASSIGNED);resource.setAssignedAgentId("owner-1");resource.setLastCustomerMessageAt(Instant.now());resource=resources.saveAndFlush(resource);
  var conversation=new ConversationEntity();conversation.setResourceId(resource.getId());conversation.setCustomerPhone(resource.getCustomerPhone());conversation.setAssignedAgentId("owner-1");conversation.setStatus(PersistenceEnums.ConversationStatus.HUMAN_ACTIVE);conversation.setFirstCustomerMessageAt(Instant.now().minusSeconds(5));conversation.setFirstAgentReplyAt(Instant.now());conversations.saveAndFlush(conversation);
  mvc.perform(get("/api/v2/dashboard/summary?scope=mine").header("Authorization","Bearer "+ownerToken())).andExpect(status().isOk()).andExpect(jsonPath("$.data.activeConversations").value(org.hamcrest.Matchers.greaterThanOrEqualTo(1))).andExpect(jsonPath("$.data.pendingAssignments").exists()).andExpect(jsonPath("$.data.expiringReplyWindows").exists()).andExpect(jsonPath("$.data.todayReceived").value(org.hamcrest.Matchers.greaterThanOrEqualTo(1))).andExpect(jsonPath("$.data.firstHumanResponseP50Seconds").value(org.hamcrest.Matchers.greaterThanOrEqualTo(4.0))).andExpect(jsonPath("$.data.firstHumanResponseP50Ms").doesNotExist());
 }
 ...
 private String ownerToken(){return tokens.generateToken(CrmAgentAccount.builder().rowId("owner-1").loginAccount("owner").role(AgentRole.OWNER).enabled(true).build());}
}
