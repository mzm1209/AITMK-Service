...
public class ResourceQueryService {
    ...
    private final AssignmentRecordRepository assignments; private final ChatMessageRepository messages; private final V2AccessService access; private final EntityManager em;
    ...
    @Transactional(readOnly=true) public ResourceView view(Long id,AuthenticatedUser u){var resource=get(id,u);var lastMessage=messages.findFirstByResourceIdOrderByCreatedAtDescIdDesc(id).orElse(null);return V2Mapper.resource(resource,lastMessage);}
    ...
    @Transactional(readOnly=true) public CursorPage<AssignmentView> assignments(Long id,String cursor,int requested,AuthenticatedUser u){get(id,u);int size=bounded(requested);String sql="select id from assignment_record where resource_id=:resourceId";CursorCodec.Key k=decode(cursor);if(k!=null)sql+=" and (assigned_at<:at or (assigned_at=:at and id<:id))";sql+=" order by assigned_at desc,id desc";var q=em.createNativeQuery(sql).setParameter("resourceId",id);if(k!=null){q.setParameter("at",k.at());q.setParameter("id",k.id());}q.setMaxResults(size+1);@SuppressWarnings("unchecked")List<Number> ids=q.getResultList();boolean more=ids.size()>size;if(more)ids=ids.subList(0,size);var rows=ids.stream().map(n->assignments.findById(n.longValue()).orElseThrow()).toList();return new CursorPage<>(rows.stream().map(V2Mapper::assignment).toList(),rows.isEmpty()?null:encode(rows.get(rows.size()-1).getAssignedAt(),rows.get(rows.size()-1).getId()),more);}
